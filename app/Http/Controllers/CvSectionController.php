...
class CvSectionController extends Controller
{
    ...
    public function getSectionQuestions()
    {
        ...
        $formattedSections = $sections->map(function ($section) {
            return [
                'section_id'        => $section->sectionQuestions->resource_section_id,
    ...
    public function getUserSectionDetailss(Request $request)
    {
        ...
        // Step 1: Get all sections tied to this user/profile
        ...
        if ($sections->isEmpty()) {
            return $this->errorResponse('No sections found for this user.', 404);
        }
    ...
    public function getUserSectionDetails(Request $request)
    {
        ...
        // Step 1: Get all profile IDs for this user
        $profiles = DB::table('create-cvprofilesection')
            ->where('id', $user_id)
            ->pluck('cvid')
            ->unique();

        if ($profiles->isEmpty()) {
            return $this->errorResponse('No profiles found for this user.', 404);
        }

        $finalResults = [];

        // Step 2: For each profile, get section details
        foreach ($profiles as $profile_id) {
            ...
            foreach ($sections as $sectionEntry) {
                ...
                // Step 3: Get section config
                $sectionConfig = DB::table('resource-section')->where('status', 1)->where('id', $section_id)->first();
                ...
                // Step 4: Retrieve data from the section table
                $sectionData = DB::table($sectionTable)
                    ->whereIn($idColumnName, $subsectionIds)
                    ->get();
            ...
            // Add profile + its sections to final results
            $finalResults[] = [
